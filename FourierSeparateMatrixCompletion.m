function [rec_img, rel_error] = FourierSeparateMatrixCompletion(sampled_image, mask, opts)
...
%%%%%%%%% MODIFIED TO WORK ON THE FOURIER TRANSFORM OF THE IMAGE, NOT
%%%%%%%%% FUNCTIONAL YET
...
rel_error = zeros(kmax, channels);
sampled_image_fft = zeros(shape);
for c = 1:channels
    sampled_image_fft(:,:,c) = fft2(sampled_image(:,:,c));
end

%loop over separate channels
parfor c = 1:channels
Y = k0 * delta * fft2(sampled_image(:,:,c));
...
%SVT iterations
    for k = 1:kmax
       ...
       [U,S,V] = svd(Y);   %do the SVD
        ...
        Z(:,:,c) = U*S*V';  %store fft'd matrix
        X(:,:,c) = ifft2(Z(:,:,c));   %construct low rank approximation
   
        %norm(mask.*(X(:,:,c)-sampled_image(:,:,c)), 'fro') / sampled_image_norm(:,:,c)
        ...
        %gradient step
        Y = Y + delta * mask.* (sampled_image_fft(:,:,c) - Z(:,:,c));  %use the fft'd Z to not do multiple ifft(fft(*)) ops
